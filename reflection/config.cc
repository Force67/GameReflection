...
std::string ConfigPath(char* arg0) {
  ...
  std::string path = sys::fs::getMainExecutable(arg0, main_address);
  SmallString<256> buf{sys::path::parent_path(path)};
  sys::path::append(buf, kConfigFileName);
  return std::string(buf.str());
}

// positional args don't start with those two characters.
bool IsPositionalArg(const StringRef ref) {
...
void InitOpts(int argc, char** argv, const char* const desc) {
  ...
  SmallVector<const char*, kMaxArgs> new_args;
  auto unique_add = [&new_args](const StringRef ref) {
  ...
  SmallVector<const char*, 16> cfg_args;
